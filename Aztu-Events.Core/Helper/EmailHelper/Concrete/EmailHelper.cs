...
namespace Aztu_Events.Core.Helper.EmailHelper.Concrete
{
    public class EmailHelper : IEmailHelper
    {
        ...
        public async Task<IResult> ApproveConfransSendEmail(string userEmail, string name, string dateTime, string AuditoriumNumber, bool UpdateDate = false)
        {
        ...
        public async Task<IResult> ApproveConfransSendEmailForGuest(string userEmail, string name, string dateTime, string AuditoriumNumber,string confransDetailUrl, bool UpdateDate = false, bool SendEmailGuest = false)
        {
            string htmlBody = string.Empty;
            ...
            else if(!SendEmailGuest&& !UpdateDate)
            {
                ...
                htmlBody = $"Hörmətli {name} sizi  konfrasa dəvət edirik.Kecirilmə tarixi və mekani:<br/> tarix: {dateTime}<br/> Məkan:{AuditoriumNumber}<br/> Daha Ətraflı Kecid linki:<br/><a class='btn btn-primary' href='{confransDetailUrl}' role='button' style='display: inline-block; padding: 6px 12px; font-size: 14px; font-weight: 400; line-height: 1.42857143; text-align: center; white-space: nowrap; vertical-align: middle; cursor: pointer; border: 1px solid transparent; border-radius: 4px; text-decoration: none; color: #fff; background-color: #337ab7; border-color: #2e6da4;'>Keçid Linki</a> <br/> Diqqətiniz üçün təşəkkürlər.";
            }
            else if(UpdateDate && !SendEmailGuest)
            {
                htmlBody = $"Hörmətli {name} sizi  konfrasa dəvət edirik.Kecirilmə tarixi və mekani:<br/> tarix: {dateTime}<br/> Məkan:{AuditoriumNumber}<br/> Daha Ətraflı Kecid linki:<br/><a class='btn btn-primary' href='{confransDetailUrl}' role='button' style='display: inline-block; padding: 6px 12px; font-size: 14px; font-weight: 400; line-height: 1.42857143; text-align: center; white-space: nowrap; vertical-align: middle; cursor: pointer; border: 1px solid transparent; border-radius: 4px; text-decoration: none; color: #fff; background-color: #337ab7; border-color: #2e6da4;'>Keçid Linki</a> <br/> Diqqətiniz üçün təşəkkürlər.";
